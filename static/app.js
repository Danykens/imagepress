...
function render() {
  ...
  $('start').disabled = running || !items.some(i => ['waiting','error'].includes(i.status));
  $('clear').disabled = running;
  $('stop').hidden = !running;
  for (const id of ['format','quality','size','keep','files']) $(id).disabled = running || (id === 'quality' && $('format').value === 'png');
  $('list').replaceChildren(...items.map(item => {
    ...
    row.append(img, details, badge);
    if (item.result) { const a = document.createElement('a'); a.href = `/api/file/${item.result.id}`; a.textContent = '↓'; a.title = `Скачать ${item.result.name}`; row.append(a); }
    return row;
  }));
...
$('start').onclick = async () => {
  ...
  items.filter(i => i.status === 'error').forEach(i => i.status = 'waiting');
  const settings = {format:$('format').value, quality:$('quality').value, size:$('size').value, keep:$('keep').checked};
  render();
  try {
    await config;
    const worker = async () => {
